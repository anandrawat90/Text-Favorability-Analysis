...
model.add(CuDNNGRU(lstm_out, go_backwards=True))
...
tb = TensorBoard(log_dir='./summary/TL_cuDnnGRU_glove', histogram_freq=8, batch_size=batch_size,
                 write_graph=True, write_grads=False, write_images=False, embeddings_freq=0,
                 embeddings_layer_names=None, embeddings_metadata=None)
hist = model.fit(X_train, Y_train, epochs = 8, batch_size=batch_size, verbose = 1,
                 callbacks=[tb],validation_data=(X_validate,Y_validate))
model.save('./models/model_TL_GRU_GloVe.h5')
